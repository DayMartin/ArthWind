...
import {
  ...
  MusicoDetalhe,
  MusicoUpdate,
} from "@/shared/interfaces/MusicoInterface";
...
const MusicosListagem = () => {
  ...
  const [selectedMusico, setSelectedMusico] = useState<MusicoDetalhe | null>(
    null
  );
  ...
  const listarMusicos = async () => {
    setIsLoading(true);
    try {
      const consulta = await MusicoService.findAllMusicos(
        page + 1,
        limit,
        filterId
      );
  ...
  const handleSave = async (updatedMusico: MusicoDetalhe) => {
    try {
      const { instrumentos, ...updatedMusicoWithoutInstrumentos } = updatedMusico;
      await MusicoService.updateMusico(updatedMusicoWithoutInstrumentos.id, updatedMusicoWithoutInstrumentos);
      alert("Músico atualizado com sucesso");
      console.log("updatedMusico Listagem", updatedMusicoWithoutInstrumentos);
        await listarMusicos();
    } catch (error) {
                ...
                <TableRow key={musico.id}>
                  ...
                  <TableCell>
                    ...
                    {musico.status === "Ativo" ? (
                      ...
                      <Button
                        onClick={() => handleStatusMusico(musico.id, "Ativo")}
                      >
                        Ativar
                      </Button>
                    )}
