...
const NovoEvento: React.FC<NovoEventoProps> = ({
  open,
  onClose,
  title,
  onSubmit,
  ...
  const listarMusicos = async () => {
    try {
      ...
      if (consulta instanceof Error) {
        ...
        const musicosAtivos = consulta.filter(
          (musico) => musico.status === "Ativo"
        );
  ...
  const listarInstrumentoMusico = async (idMusico: number) => {
    try {
      const consulta = await MusicoInstrumento.findInstrumentosByMusico(
        idMusico
      );
      if (consulta instanceof Error) {
        setInstrumentos([]);
  ...
  const handleRemoveMusico = (musicoId: number, instrumentoId: number) => {
    ...
    setFormData((prevData) => ({
      ...
      musicos: prevData.musicos.filter(
        (musico) =>
          musico.id !== musicoId || musico.instrumento !== instrumentoId
      ),
      valor_total: prevData.valor_total - valorMusico,
    }));
  };
                ...
                <Grid item xs={12}>
                  <FormControl fullWidth margin="normal">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setSelectedInstrumento(Number(e.target.value))
                      }
                      label="Instrumentos"
                    >
                ...
                <List>
                  {formData.musicos.map((musico, index) => (
                    <ListItem key={index}>
                      <ListItemText
                        primary={`Músico: ${
                          musicos.find((m) => m.id === musico.id)?.fullName
                        }`}
                        secondary={`Instrumento: ${
                          instrumentos.find((i) => i.id === musico.instrumento)
                            ?.instrumento.name
                        }`}
                      />
                      <IconButton
                        edge="end"
                        onClick={() =>
                          handleRemoveMusico(musico.id, musico.instrumento)
                        }
                      >
                        <DeleteIcon />
                      </IconButton>
